...
# -----------------------
# Evitar clave primaria duplicada
# -----------------------
# Un mismo procedimiento_id puede aparecer en adjudicaciones con distinta
# Modalidad / Tipo de Procedimiento. El drop_duplicates() de arriba conserva
# esas variantes como filas distintas, lo que produce procedimiento_id repetido
# y rompe la PK (dim_procedimiento_pkey). Nos quedamos con un unico registro por
# procedimiento_id y, por las dudas, excluimos los que ya existen en la
# dimension (hace el paso idempotente ante re-ejecuciones / rebuilds).
faltantes_dim = faltantes_dim.drop_duplicates(
    subset=["procedimiento_id"],
    keep="first"
)

faltantes_dim = faltantes_dim[
    ~faltantes_dim["procedimiento_id"].isin(dim_proc["procedimiento_id"])
]
